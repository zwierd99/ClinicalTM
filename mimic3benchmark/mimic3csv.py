...
def break_up_stays_by_subject(stays, output_path, subjects=None, verbose=1, sortby= 'INTIME'):
    ...
    for i, subject_id in enumerate(subjects):
        ...
        stays.loc[stays.SUBJECT_ID == subject_id].sort_values(by=sortby).to_csv(os.path.join(dn, 'stays.csv'), index=False)
    if verbose:
