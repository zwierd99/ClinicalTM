...
def split(dir):
    ...
    with open(os.path.join(dir, 'train/listfile.csv')) as listfile:
    ...
    with open(os.path.join(dir, 'train_listfile.csv'), 'w') as train_listfile:
        train_listfile.write(header)
        for line in train_lines:
            train_listfile.write(line)

    with open(os.path.join(dir, 'val_listfile.csv'), 'w') as val_listfile:
        val_listfile.write(header)
        for line in val_lines:
            val_listfile.write(line)

    shutil.copy(os.path.join(dir, 'test/listfile.csv'),
                os.path.join(dir, 'test_listfile.csv'))
